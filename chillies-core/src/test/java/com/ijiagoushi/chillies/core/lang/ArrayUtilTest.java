...
public class ArrayUtilTest {
    ...
    public void toPrimitive() {
        ...
        ca = new Character[0];
        assertArrayEquals(ArrayUtil.toPrimitive(ca), new char[0]);

        ca = new Character[]{' '};
        assertArrayEquals(ArrayUtil.toPrimitive(ca), new char[]{' '});

        Boolean[] ba = null;
        assertNull(ArrayUtil.toPrimitive(ba));

        ba = new Boolean[0];
        assertArrayEquals(ArrayUtil.toPrimitive(ba), new boolean[0]);

        ba = new Boolean[]{true};
        assertArrayEquals(ArrayUtil.toPrimitive(ba), new boolean[]{true});

        Byte[] byteA = null;
        assertNull(ArrayUtil.toPrimitive(byteA));

        byteA = new Byte[0];
        assertArrayEquals(ArrayUtil.toPrimitive(byteA), new byte[0]);

        byteA = new Byte[]{'a'};
        assertArrayEquals(ArrayUtil.toPrimitive(byteA), new byte[]{97});

        Short[] shortA = null;
        assertNull(ArrayUtil.toPrimitive(shortA));

        shortA = new Short[0];
        assertArrayEquals(ArrayUtil.toPrimitive(shortA), new short[0]);

        shortA = new Short[]{10};
        assertArrayEquals(ArrayUtil.toPrimitive(shortA), new short[]{10});

        Integer[] intArray = null;
        assertNull(ArrayUtil.toPrimitive(intArray));

        intArray = new Integer[0];
        assertArrayEquals(ArrayUtil.toPrimitive(intArray), new int[0]);

        intArray = new Integer[]{1, 23};
        assertArrayEquals(ArrayUtil.toPrimitive(intArray), new int[]{1, 23});

        Long[] longA = null;
        assertNull(ArrayUtil.toPrimitive(longA));

        longA = new Long[0];
        assertArrayEquals(ArrayUtil.toPrimitive(longA), new long[0]);

        longA = new Long[]{11111L};
        assertArrayEquals(ArrayUtil.toPrimitive(longA), new long[]{11111});

        Float[] floatA = null;
        assertNull(ArrayUtil.toPrimitive(floatA));

        floatA = new Float[0];
        assertArrayEquals(ArrayUtil.toPrimitive(floatA), new float[0]);

        floatA = new Float[]{0.0f};
        assertArrayEquals(ArrayUtil.toPrimitive(floatA), new float[]{0.0f});

        Double[] doubleA = null;
        assertNull(ArrayUtil.toPrimitive(doubleA));

        doubleA = new Double[0];
        assertArrayEquals(ArrayUtil.toPrimitive(doubleA), new double[0]);

        doubleA = new Double[]{0.0};
        assertArrayEquals(ArrayUtil.toPrimitive(doubleA), new double[]{0.0});
    ...
    public void toObject() {
        ...
        ca = new char[0];
        assertArrayEquals(ArrayUtil.toObject(ca), new Character[0]);

        ca = new char[]{' '};
        assertArrayEquals(ArrayUtil.toObject(ca), new Character[]{' '});

        boolean[] ba = null;
        assertNull(ArrayUtil.toObject(ba));

        ba = new boolean[0];
        assertArrayEquals(ArrayUtil.toObject(ba), new Boolean[0]);

        ba = new boolean[]{true};
        assertArrayEquals(ArrayUtil.toObject(ba), new Boolean[]{true});

        byte[] byteA = null;
        assertNull(ArrayUtil.toObject(byteA));

        byteA = new byte[0];
        assertArrayEquals(ArrayUtil.toObject(byteA), new Byte[0]);

        byteA = new byte[]{'a'};
        assertArrayEquals(ArrayUtil.toObject(byteA), new Byte[]{97});

        short[] shortA = null;
        assertNull(ArrayUtil.toObject(shortA));

        shortA = new short[0];
        assertArrayEquals(ArrayUtil.toObject(shortA), new Short[0]);

        shortA = new short[]{10};
        assertArrayEquals(ArrayUtil.toObject(shortA), new Short[]{10});

        int[] intArray = null;
        assertNull(ArrayUtil.toObject(intArray));

        intArray = new int[0];
        assertArrayEquals(ArrayUtil.toObject(intArray), new Integer[0]);

        intArray = new int[]{1, 23};
        assertArrayEquals(ArrayUtil.toObject(intArray), new Integer[]{1, 23});

        long[] longA = null;
        assertNull(ArrayUtil.toObject(longA));

        longA = new long[0];
        assertArrayEquals(ArrayUtil.toObject(longA), new Long[0]);

        longA = new long[]{11111};
        assertArrayEquals(ArrayUtil.toObject(longA), new Long[]{11111L});

        float[] floatA = null;
        assertNull(ArrayUtil.toObject(floatA));

        floatA = new float[0];
        assertArrayEquals(ArrayUtil.toObject(floatA), new Float[0]);

        floatA = new float[]{0.0f};
        assertArrayEquals(ArrayUtil.toObject(floatA), new Float[]{0.0f});

        double[] doubleA = null;
        assertNull(ArrayUtil.toObject(doubleA));

        doubleA = new double[0];
        assertArrayEquals(ArrayUtil.toObject(doubleA), new Double[0]);

        doubleA = new double[]{0.0};
        assertArrayEquals(ArrayUtil.toObject(doubleA), new Double[]{0.0});
    ...
    public void reverse1() {
        ...
        array = new String[]{};
        assertArrayEquals(ArrayUtil.reverse(array), new String[]{});

        array = new String[]{"1", "2"};
        String[] actualArray = ArrayUtil.reverse(array);
        assertArrayEquals(actualArray, new String[]{"2", "1"});
