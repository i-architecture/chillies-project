...
import com.ijiagoushi.chillies.easypoi.utils.PoiUtils;
 ...
 */
class ExcelReaderEngine<E> {
    //slf4j
    private final Logger logger = LoggerFactory.getLogger(ExcelReaderEngine.class);
    //导入的配置选项
    ...
    ExcelReaderEngine(ReadExcelConfig readExcelConfig) {
        this.readExcelConfig = readExcelConfig;
        if (ClassUtil.isAssignable(Map.class, this.readExcelConfig.getRawClass())) {
...
//                in = ExcelType.XLS;
            ...
            //按照sheet读取
            for (int i = this.readExcelConfig.getSheetIndex() - 1; i < this.readExcelConfig.getSheetIndex() + this.readExcelConfig.getSheetNum() - 1; i++) {
    ...
    private List<ReadExcelCellHeaderConfig> convertByJavaBean(Class<?> beanClass, String[] includePropertyNames) {
        ...
        List<Field> fieldList = ClassUtil.getDeclaredFields(beanClass);
        if (includePropertyNames != null && includePropertyNames.length == 1 && includePropertyNames[0].equals("*")) {
            includePropertyNames = null;
        }

        for (Field field : fieldList) {
            if (includePropertyNames != null && !ArrayUtil.containsElement(includePropertyNames, field.getName())) {
                continue;
            }
    ...
    private List<ReadExcelCellHeaderConfig> mergeCellHeaders(List<InternalExcelHeader> internalExcelHeaderList) {
        ...
        for (InternalExcelHeader internalExcelHeader : internalExcelHeaderList) {
            ...
            if (CollectionUtil.isNotEmpty(childExcelHeaders)) {
                List<ReadExcelCellHeaderConfig> _children = readExcelCellHeaderConfig.children;
                if (CollectionUtil.isEmpty(_children)) {
    ...
    private List<InternalExcelHeader> getExcelHeaders(Iterator<Row> rowIterator) {
        ...
        for (InternalExcelHeader excelHeader : internalExcelHeaderList) {
            ...
            if (CollectionUtil.isNotEmpty(children)) excelHeader.childExcelHeaders = children;
        }
        return internalExcelHeaderList;
    }
