...
import com.ijiagoushi.chillies.easypoi.exceptions.ReadExcelException;
...
public class ReadExcelConfig extends ExcelEntityValidator {
    ...
    private ReadExcelConfig(Builder builder) {
        ...
        //validate
        Preconditions.checkArgument(this.headerRowStart >= 0, "Property [headerRowStart] must be great than 0.");
       Preconditions.checkArgument(this.headerRowUsed > 0, "Property [headerTitleRowUsed] must be great than 0.");
       Preconditions.checkArgument(this.dataRowGap >= 0, "Property [DataRowGap] must be great than 0.");
       Preconditions.checkArgument(this.sheetIndex > 0, "Property [SheetIndex] must be great than 0.");
       Preconditions.checkArgument(this.sheetNum > 0, "Property [SheetNum] must be great than 0.");
       Preconditions.checkArgument(this.lastInvalidRow >= 0, "LastInvalidRow must be great than 0.");
        if (ClassUtil.isAssignable(Map.class, this.rawClass)) {
            Preconditions.requireNonNull(this.cellHeaderConfigList, "Property [cellHeaderConfigList] must be not null or empty if raw class is Map.");
        } else {
    ...
    public static class Builder {
        ...
        public Builder excelCellHeaderConfig(ExcelCellHeaderConfig excelCellHeaderConfig) {
            ...
            if (StringUtil.isEmpty(excelCellHeaderConfig.getTitle())) {
                throw new ReadExcelException("ExcelCellHeaderConfig title is null.");
            }
            if (StringUtil.isEmpty(excelCellHeaderConfig.getPropertyName())) {
