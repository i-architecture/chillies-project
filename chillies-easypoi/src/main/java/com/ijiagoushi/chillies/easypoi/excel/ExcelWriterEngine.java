...
class ExcelWriterEngine {
    ...
    ExcelWriterEngine(ExcelWriterConfig excelWriterConfig) {
        ...
        if (ClassUtil.isAssignable(Map.class, excelWriterConfig.getRawClass())) {
    ...
    <E> void write(Collection<E> dataSet, OutputStream out) {
        ...
        try {
            ...
            Sheet sheet;
            if (StringUtil.hasText(this.excelWriterConfig.getSheetName())) {
    ...
    private int createTitleRow(Sheet sheet) {
        if (StringUtil.isEmpty(this.excelWriterConfig.getTitle())) return 0;
    ...
    private int createSecondTitleRow(Sheet sheet, int rowIndex) {
        if (StringUtil.isEmpty(this.excelWriterConfig.getSecondTitle())) return 0;
